...
export const fullSizeImagePath = path.resolve(__dirname, '../../images')
export const thumbnailImagePath = path.resolve(__dirname, '../../images/thumbnails')
...
export const getFileNameWithSize = (props: ImageProps): string | boolean => {
  ...
  const widthValue = convertNumberValue(width)
  const heightValue = convertNumberValue(height)
...
export const getImagePath = async (props: ImageProps): Promise<string | boolean> => {
  ...
  const isThumbnail = isThumbnailImage(newFileName)
